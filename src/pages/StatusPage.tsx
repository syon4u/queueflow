...
const StatusPage: React.FC = () => {
  ...
  return <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            ...
            <div className="flex space-x-2 mb-4">
              <Button type="button" variant={lookupMethod === 'confirmation' ? 'default' : 'outline'} size="sm" onClick={() => setLookupMethod('confirmation')} className="flex-1">
                Confirmation #
              </Button>
              <Button type="button" variant={lookupMethod === 'details' ? 'default' : 'outline'} size="sm" onClick={() => setLookupMethod('details')} className="flex-1">
                Name & Phone
              </Button>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              {lookupMethod === 'confirmation' ? <div>
                  <label htmlFor="confirmation" className="block text-sm font-medium text-gray-700 mb-1">
                    Confirmation Number
                  </label>
                  <Input id="confirmation" type="text" placeholder="Enter CUST-XXXXXXXX or APT-XXXXXXXX" value={confirmationNumber} onChange={e => setConfirmationNumber(e.target.value)} autoFocus className="uppercase" />
                  ...
                  <div>
                    ...
                    <Input id="lastName" type="text" placeholder="Enter your last name" value={lastName} onChange={e => setLastName(e.target.value)} autoFocus />
                  </div>
                  <div>
              ...
              <Button type="submit" className="w-full" disabled={isLoading || (lookupMethod === 'confirmation' ? !confirmationNumber.trim() : !lastName.trim() || !phone.trim())}>
        ...
        {/* Error Results */}
        {error && <Card className="mt-6 border-red-200 bg-red-50">
        ...
        {/* Success Results */}
        {statusData && <Card className="mt-6 shadow-sm" role="region" aria-live="polite" aria-label="Queue status results">
            <CardContent className="pt-6">
              <div className="space-y-6">
                ...
                {statusData.is_checked_in && statusData.position && <div className="text-center bg-blue-50 rounded-lg p-4">
                    <div className="text-4xl font-bold text-blue-600 mb-1">
                      #{statusData.position}
                    </div>
                    <p className="text-gray-600">Your position in line</p>
                    {statusData.estimated_wait_time_minutes > 0 && <div className="flex items-center justify-center mt-2 text-blue-600">
                ...
                <div className="space-y-3 bg-gray-50 rounded-lg p-4">
                  ...
                  {statusData.check_in_time && <div className="flex items-start">
                ...
                {/* Action Buttons */}
                {!statusData.is_checked_in && statusData.status === 'scheduled' && <div className="text-center">
                    <Link to="/customer">
                ...
                {/* Ticket Number */}
                {statusData.ticket_number && <div className="text-center pt-2 border-t border-gray-200">
