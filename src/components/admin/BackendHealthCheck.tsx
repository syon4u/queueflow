...
const BackendHealthCheck: React.FC = () => {
  ...
  const runHealthChecks = async () => {
    ...
    // 3. Test Tables Access
    const tables = ['locations', 'services', 'staff', 'customers', 'appointments', 'user_roles'];
    
    for (const table of tables) {
      updateResult(`Table: ${table}`, 'loading', 'Checking access...');
      try {
        const { data, error } = await supabase.from(table).select('count').limit(1);
        if (error) throw error;
        updateResult(`Table: ${table}`, 'success', 'Accessible');
      } catch (error) {
        updateResult(`Table: ${table}`, 'error', `Access denied: ${error.message}`);
      }
    }
